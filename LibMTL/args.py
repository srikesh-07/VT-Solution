...
_parser.add_argument(
    "--arch", type=str, default="HPS", help="architecture for MTL, option: HPS, MTAN"
)
_parser.add_argument(
    "--rep_grad",
    action="store_true",
...
_parser.add_argument("--save_path", type=str, default=None, help="save path")
_parser.add_argument("--load_path", type=str, default=None, help="load ckpt path")
## optim
_parser.add_argument(
    "--optim",
    type=str,
...
_parser.add_argument(
    "--lr", type=float, default=1e-4, help="learning rate for all types of optim"
)
...
_parser.add_argument(
    ...
    type=float,
    default=1e-5,
    help="weight decay for all types of optim",
)
## scheduler
_parser.add_argument(
    "--scheduler",
...
_parser.add_argument(
    ...
    type=int,
    default=1,
    help="update_weights_every for Nash_MTL",
)
_parser.add_argument(
    "--optim_niter", type=int, default=20, help="optim_niter for Nash_MTL"
)
_parser.add_argument(
    "--max_norm", type=float, default=1.0, help="max_norm for Nash_MTL"
)
## MoCo
_parser.add_argument("--MoCo_beta", type=float, default=0.5, help="MoCo_beta for MoCo")
_parser.add_argument(
    "--MoCo_beta_sigma", type=float, default=0.5, help="MoCo_beta_sigma for MoCo"
)
_parser.add_argument("--MoCo_gamma", type=float, default=0.1, help="gamma for MoCo")
_parser.add_argument(
    "--MoCo_gamma_sigma", type=float, default=0.5, help="MoCo_gamma_sigma for MoCo"
)
...
_parser.add_argument(
    "--num_experts",
    nargs="+",
    help="the number of experts for sharing and task-specific",
)
## DSelect_k
_parser.add_argument(
    "--num_nonzeros", type=int, default=2, help="num_nonzeros for DSelect-k"
)
...
def prepare_args(params):
        ...
        if params.weighting in ["DWA"]:
            ...
            if params.mgda_gn is not None:
                if params.mgda_gn in ["none", "l2", "loss", "loss+"]:
                    ...
                    raise ValueError(
                        "No support mgda_gn {} for MGDA".format(params.mgda_gn)
                    )
            else:
            ...
            if (
                params.update_weights_every is not None
                and params.optim_niter is not None
                and params.max_norm is not None
            ):
                kwargs["weight_args"][
                    "update_weights_every"
                ] = params.update_weights_every
                kwargs["weight_args"]["optim_niter"] = params.optim_niter
                kwargs["weight_args"]["max_norm"] = params.max_norm
            else:
                raise ValueError(
                    "Nash_MTL needs update_weights_every, optim_niter, and max_norm"
                )
    ...
    if params.arch in [
        ...
        if params.arch in ["CGC", "PLE", "MMoE", "DSelect_k"]:
            kwargs["arch_args"]["img_size"] = tuple(
                params.img_size
            )  # np.array(params.img_size, dtype=int).prod()
            kwargs["arch_args"]["num_experts"] = [
                int(num) for num in params.num_experts
            ]
...
def _display(params, kwargs, optim_param, scheduler_param):
    ...
    print(
        "\tDevice: {}".format(
            "cuda:" + params.gpu_id if torch.cuda.is_available() else "cpu"
        )
    )
...
def get_weight_args(name: str):
    a = LibMTL_args.parse_args("")
    a.weighting = name
    return prepare_args(a)[0]["weight_args"]
