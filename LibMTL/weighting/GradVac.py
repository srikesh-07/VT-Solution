...
class GradVac(AbsWeighting):
        ...
        if group_type == 0:  # whole_model
            ...
            for module in self.encoder.modules():
                if len(module._modules.items()) == 0 and len(module._parameters) > 0:
                    self.k_idx.append(
                        sum([w.data.numel() for w in module.parameters()])
                    )
            ...
            raise ValueError
        self.rho_T = torch.zeros(self.task_num, self.task_num, len(self.k_idx)).to(
            self.device
        )
    ...
    def backward(self, losses, **kwargs):
        ...
        if self.rep_grad:
            raise ValueError(
                "No support method GradVac with representation gradients (rep_grad=True)"
            )
        else:
            self._compute_grad_dim()
        ...
        for tn_i in range(self.task_num):
            ...
            for tn_j in task_index:
                for k in range(len(self.k_idx)):
                    ...
                    rho_ijk = torch.dot(
                        pc_grads[tn_i, beg:end], grads[tn_j, beg:end]
                    ) / (
                        pc_grads[tn_i, beg:end].norm() * grads[tn_j, beg:end].norm()
                        + 1e-8
                    )
