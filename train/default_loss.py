# This loss is obtained from the https://github.com/wildoctopus/cbloss.
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, num_classes, alpha=1, gamma=1.0, reduction="mean"):
        super().__init__()
    ...
    def forward(self, outputs, targets):
        ...
        if self.reduction == "mean":
            ...
            raise ValueError(
                f"Invalid reduction method: {self.reduction}. Please use 'mean' or 'sum'."
            )
...
class ClassBalancedLoss(nn.Module):
    ...
    def __init__(self, loss_func, weight=None, samples_per_cls=None, beta=0.9999):
        super().__init__()
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.loss_func = loss_func
        if weight is not None:
            self.weight = torch.tensor(weight, device=self.device).float()
        else:
            effective_num = 1.0 - np.power(beta, samples_per_cls)
            self.weight = (1.0 - beta) / np.array(effective_num)
            self.weight = self.weight / np.sum(self.weight) * len(samples_per_cls)
            self.weight = torch.tensor(self.weight, device=self.device).float()
    ...
    def forward(self, logits, target):
        ...
        if self.loss_func.reduction == "none":
            ...
            weights = self.weight.index_select(0, target)
            balanced_loss = (weights * base_loss).mean()

            return balanced_loss
        else:
            raise ValueError(
                f"Invalid reduction method: {self.loss_func.reduction}. Please use 'none'."
            )
